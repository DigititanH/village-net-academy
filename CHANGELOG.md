...
- Injected `VITE_API_URL` environment variable support into the auto-generated Azure Static Web Apps workflow (`azure-static-web-apps-*.yml`) to prevent blank API URL builds from causing `405 Method Not Allowed` errors on API requests.
- Documented deployment requirements for setting the API URL to end with `/api` in GitHub Secrets to align with backend routing paths.
