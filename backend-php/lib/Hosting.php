...
class Hosting
{
    ...
    {
        $required = ['json', 'mbstring', 'curl', 'fileinfo', 'openssl'];
    ...
    public static function productionConfigErrors(): array
    {
        ...
        if (!Env::envFileFound()) {
            $errors[] = 'backend-php/.env was not found or is not readable. Create it next to the public folder (not inside public). Do not name it .env.txt';
        }
        ...
        if (!Env::get('DATABASE_PATH')) {
            ...
            if ($dbUser === '' || $dbUser === 'root') {
                $errors[] = 'DB_USER must be your cPanel MySQL user (e.g. villagenetacad_admin), not root';
            }
            if (!array_key_exists('DB_PASSWORD', $_ENV) && getenv('DB_PASSWORD') === false) {
                $errors[] = 'DB_PASSWORD must be set in .env';
            }
        }
    ...
    public static function healthPayload(): array
    {
        ...
        $payfast = Payfast::isConfigured()
            ? array_merge(Payfast::statusPayload(), [
                'donations_enabled' => true,
                'shop_enabled' => true,
            ])
            : ['configured' => false, 'donations_enabled' => false, 'shop_enabled' => false];
