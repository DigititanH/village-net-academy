...
class DonationsController
{
    public static function create(): void
    {
        ...
        if ($payfastEnabled && $amount < 5) {
            Response::error('PayFast donations must be at least R5.00', 400);
        }
        ...
        $donorLabel = $isAnonymous ? 'Anonymous' : ($donorName ?: 'Donor');
        ...
        if ($payfastEnabled) {
            try {
                $itemDescription = 'Donation for ' . $academy;
                if ($message !== '') {
                    $itemDescription .= ' — ' . $message;
                }
                $fields = Payfast::buildPaymentPayload([
                    'amount' => $amount,
                    'itemName' => 'Village Netacad Donation',
                    ...
                    'customStr1' => $academy,
                    'customStr2' => $message !== '' ? $message : null,
                ]);
            } catch (Throwable $e) {
                Database::queryRun('DELETE FROM donations WHERE id = ?', [$donationId]);
