...
class PayfastController
{
    ...
    public static function check(): void
    {
        ...
        if (Payfast::isLocalUrl((string) ($sample['return_url'] ?? ''))) {
    ...
    public static function notify(): void
    {
        ...
        try {
            ...
            // ITN: hash posted fields in the order PayFast sent them (not form attribute order).
            ...
            if ($parsed['type'] === 'order') {
                ...
                if ($paidAmount > 0 && abs($paidAmount - (float) $donation['amount']) > 0.01) {
                    error_log("PayFast ITN: donation amount mismatch $paidAmount vs {$donation['amount']}");
                    return;
                }
                ...
                if ($paidAmount > 0 && abs($paidAmount - (float) $enrolment['amount']) > 0.01) {
                    error_log("PayFast ITN: CCNA amount mismatch $paidAmount vs {$enrolment['amount']}");
                    return;
                }
