...
try {
    ...
    $raw = $e->getMessage();
    $isDbAuth = stripos($raw, 'Access denied for user') !== false
        || stripos($raw, 'SQLSTATE[HY000] [1045]') !== false
        || stripos($raw, 'Unknown database') !== false;

    if ($isProduction && $isDbAuth) {
        Response::json([
            'message' => 'Database login failed. Fix DB_NAME, DB_USER, and DB_PASSWORD in backend-php/.env to match cPanel MySQL Databases exactly, then check /health.',
            'hint' => 'https://www.villagenetacad.co.za/health',
        ], 500);
    }
