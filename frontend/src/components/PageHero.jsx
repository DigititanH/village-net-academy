 ...
 * On phones: compact title only (no tall photo header) so content stays visible.
 */
export default function PageHero({
  ...
  const titleClass = titleBrand
    ? "brand-text"
    : "bg-gradient-to-r from-burnt-300 via-primary-300 to-sky-300 bg-clip-text text-transparent";

  return (
    <>
      {/* Phone: compact title strip — no tall photo header */}
      <section className="md:hidden border-b border-white/10 bg-black/40 px-4 py-6 text-center">
        {eyebrow && (
          <p className="text-burnt-300 uppercase tracking-[0.25em] text-[10px] font-semibold mb-2">
            {eyebrow}
          </p>
        )}
        <h1 className={`text-2xl font-display font-bold leading-tight ${titleClass}`}>
          {title}
        </h1>
        {subtitle && (
          <div className="mt-2 text-sm text-white/80 max-w-lg mx-auto leading-relaxed">
            {subtitle}
          </div>
        )}
      ...
      {/* Tablet / desktop: full photo header */}
      <section
        className={`relative overflow-hidden items-center hidden md:flex ${
          compact ? "min-h-[260px] py-10" : "min-h-[420px] py-16 md:py-20"
        }`}
      >
        <img
          src={image}
        ...
        <div className="relative z-10 w-full max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <h1
            ...
            } font-display font-bold leading-tight drop-shadow-lg ${titleClass}`}
          >
            {title}
          </h1>
          {subtitle && (
